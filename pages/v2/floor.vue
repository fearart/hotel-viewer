<template>
  <div class="flex flex-col p-4 items-center w-full montserrat">
      ...
      <USeparator class="prevent-select my-4 cursor-pointer " @click="toggleRooms"><div class="flex flex-col align-center"><p>Pokoje [{{ rooms.length }}]</p> <p>{{ calculateRoomsPercentage() }}% </p></div></USeparator>
      <div v-if="displayRooms"
          ...
          >
          <div class="mb-3 room-card text-white flex-col flex items-center justify-center rounded-lg cursor-pointer w-12 h-12  xl:w-24 xl:h-24 text-sm xl:text-sm 2xl:text-2xl grow bg-sky-600" @click="openRoomInfoModal">
      ...
      <USeparator class="prevent-select my-4 cursor-pointer" @click="toggleCorridor"><div class="flex flex-col align-center"><p>Korytarze [{{ corridors.length }}]</p> <p>{{ calculateCorridorsPercentage() }}% </p></div></USeparator>
      <div v-if="displayCorridor"class="grid 2xl:grid-cols-12 xl:grid-cols-10 grid-cols-6 w-full place-items-center justify-center h-full">
        <div class="mb-3 room-card text-white flex-col flex items-center justify-center rounded-lg cursor-pointer w-12 h-12  xl:w-24 xl:h-24 text-sm xl:text-sm 2xl:text-2xl grow bg-sky-600">
      ...
      <USeparator class="prevent-select my-4 cursor-pointer" @click="toggleCinemas">Kina</USeparator>
      ...
      <USeparator class="prevent-select my-4 cursor-pointer" @click="toggleConferenceRooms">Sale konferencyjne</USeparator>
      <USeparator class="prevent-select my-4 cursor-pointer" @click="toggleRestaurants">Restauracje</USeparator>
      <USeparator class="prevent-select my-4 cursor-pointer">Bawialni</USeparator>
      <USeparator class="prevent-select my-4 cursor-pointer" @click="toggleKitchens">Kuchnie</USeparator>
      
  </div>
  <!-- rooms modal-->
  <RoomModal v-if="openedRoom" v-model:open="isOpenRoomModal" @close="handleRoomClose" @update:open="handleRoomClose" :user="user" :activeRoom="openedRoom"/>
  <RoomStatsModal v-if="isOpenRoomInfoModal" v-model:open="isOpenRoomInfoModal" :rooms="rooms" @update:open="handleRoomStatsClose"/>
  <!-- corridors modal -->
  <CorridorModal v-if="openedCorridor" v-model:open="isOpenCorridorModal" @close="handleCorridorClose" @update:open="handleCorridorClose" :user="user" :activeCorridor="openedCorridor"/>
  <CorridorStatsModal v-if="isOpenCorridorInfoModal" v-model:isOpen="isOpenCorridorInfoModal" @close="handleCorridorStatsClose" :corridor="corridors" @update:isOpen="handleCorridorStatsClose"/>
  <!-- photo gallery modal -->
  <UModal v-if="isOpenPhotoGallery" v-model:open="isOpenPhotoGallery" >
      <div class="flex flex-col p-4">
          <div class="flex justify-end w-full">
              <UButton label="Zamknij okno" icon="i-heroicons-x-mark" color="error" variant="ghost" @click="isOpenPhotoGallery = false" class="right-0 top-0 m-2"/>
          </div>
          <div v-for="(image,imageIndex) in roomImages" class="p-4">
              <div class="border-sky-400 border-y-2 flex flex-col">
                  <div class="w-full justify-end flex">
                      <UButton label="Usuń" icon="i-heroicons-x-mark" color="error" variant="ghost" @click="deleteImage(imageIndex)"></UButton>
                  </div>
  ...
  <UModal v-if="isOpenKitchenGallery" v-model="isOpenKitchenGallery" >
      <div class="flex flex-col p-4">
          <div class="flex justify-end w-full">
              <UButton label="Zamknij okno" icon="i-heroicons-x-mark" color="error" variant="ghost" @click="isOpenKitchenGallery = false" class="right-0 top-0 m-2"/>
          </div>
          <div v-for="(image,imageIndex) in kitchenImages" class="p-4">
              <div class="border-sky-400 border-y-2 flex flex-col">
                  <div class="w-full justify-end flex">
                      <UButton label="Usuń" icon="i-heroicons-x-mark" color="error" variant="ghost" @click="deleteKitchenImage(imageIndex)"></UButton>
                  </div>
                  <img :src="image" class="mt-2">
              </div>
          </div>
  ...
  <!-- Mac Search -->
  <UModal v-if="isOpenSearchForm" v-model="isOpenSearchForm" >
      <div class="p-4">
          <button></button>
...
// Objects
// @ts-ignore
const openedRoom = ref<Room>(null)
// @ts-ignore
const openedCorridor = ref<Corridor>(null)
// @ts-ignore
...
const handleRoomClose = () => {
  ...
  openedRoom.value = null // <-- Add this line
  getFloorInfo()
}
...
const handleCorridorClose= () => {
  ...
  openedCorridor.value = null // <-- Add this line
  getFloorInfo()
}
